...
export class ProcessStateService {
    ...
    getProcessStatus(): Observable<ProcessStateModel> {
        return new Observable<ProcessStateModel>((observer) => {
            this.getRawData().subscribe((obsArray: any[][]) => {
                observer.next(ProcessStateService.setProcessStatus(obsArray));
                observer.complete();
            });
        });
    }

    private static setProcessStatus(obsArray: any[][]): ProcessStateModel {
