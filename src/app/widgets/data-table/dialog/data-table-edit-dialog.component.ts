...
export class DataTableEditDialogComponent implements OnInit {
    ...
    saving = false;
    icons: string[] = [
        'power', 'power_off', 'toggle_on', 'toggle_off', 'sensor_window', 'sensor_door', 'meeting_room', 'tv', 'tv_off', 'flash_on', 'flash_off', 'emoji_objects', 'check_circle_outline', 'highlight_off'
    ];
    ...
    compareValues(a: ExportValueCharacteristicModel, b: ExportValueCharacteristicModel) {
        return a !== undefined && b !== undefined && a.Name !== undefined && b.Name !== undefined
            && a.Path !== undefined && b.Path !== undefined && a.Name === b.Name && a.Path === b.Path;
    }
    ...
    addMeasurement(measurement: DataTableElementModel | undefined, init: boolean = false) {
        ...
        newGroup.get('valueType')?.valueChanges
            .subscribe(value => {
    ...
    save(): void {
        ...
        forkJoin(observables).pipe(flatMap(() => {
    ...
    getBoundaryWarningTooltip(element: AbstractControl): string {
        ...
        const warning_enabled = wg.get('enabled') as FormControl;
        ...
        if (warning_enabled.disabled && (element.get('valueType') as FormControl).valid) {
                    ...
                    if (operatorId !== null) {
                        ...
                        if (operator !== undefined) {
                            exports = this.dataTableHelperService
                                .getExportsForPipelineOperatorValue(pipelineId, operator.name, operatorId, exportValuePath);
                        }
                    }
                }
    ...
    getOperatorValues(element: AbstractControl): ExportValueBaseModel[] {
        ...
        if (operatorId !== undefined && operatorId !== null) {
            ...
            ioModels.forEach(ioModel => values.push({
                Name: ioModel.name,
    ...
    private onServiceSelected(element: AbstractControl) {
        ...
        if (service.interaction === null || service.interaction === undefined
    ...
    private createExports(): Observable<any>[] {
        ...
        elements.forEach(element => {
            ...
            observables.push(
                this.exportService.startPipeline(preparedExport).pipe(map(model => {
    ...
    private preparePipelineOperatorExport(element: AbstractControl): ExportModel {
        ...
        if (pipeline === undefined || operator === undefined || exportValueName === undefined
            || exportValuePath === undefined || valueType === undefined) {
        ...
        elements.forEach((element, index) => {
            if (element.get('elementDetails')?.get('device')?.get('requestDevice')?.value === true) {
                ...
                const xml = '<?xml version="1.0" encoding="UTF-8"?>\n' +
                    '<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:camunda="http://camunda.org/schema/1.0/bpmn" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" id="Definitions_1" targetNamespace="http://bpmn.io/schema/bpmn"><bpmn:process id="generatedByProcessStatusWidget" isExecutable="true"><bpmn:startEvent id="StartEvent_1"><bpmn:outgoing>SequenceFlow_1oborg2</bpmn:outgoing></bpmn:startEvent><bpmn:sequenceFlow id="SequenceFlow_1oborg2" sourceRef="StartEvent_1" targetRef="Task_0os5tro" /><bpmn:endEvent id="EndEvent_131n4r3"><bpmn:incoming>SequenceFlow_0lpgosu</bpmn:incoming></bpmn:endEvent><bpmn:sequenceFlow id="SequenceFlow_0lpgosu" sourceRef="Task_0os5tro" targetRef="EndEvent_131n4r3" /><bpmn:serviceTask id="Task_0os5tro" name="' + selectedFunction.name + '" camunda:type="external" camunda:topic="pessimistic"><bpmn:extensionElements><camunda:inputOutput><camunda:inputParameter name="payload">{\n' +
                    '    "function": {\n' +
                    '        "id": "' + selectedFunction.id + '",\n' +
                    '        "name": "' + selectedFunction.name + '",\n' +
                    '        "concept_id": "' + selectedFunction.concept_id + '",\n' +
                ...
                observables.push(
                    this.deploymentsService.v2getPreparedDeploymentsByXml(xml, svg)
                        .pipe(map((pD: V2DeploymentsPreparedModel | null) => {
                            if (pD !== null && pD.elements.length === 1 && pD.elements[0].task !== null
                        ...
                        .pipe(flatMap(deployment => {
                            if (deployment !== null && deployment.status === 200) {
                                ...
                                let cron = refreshTime === '*' ? refreshTime
                                    : Math.round(((refreshTime / elements.length) * index)) as unknown as string + '/' + refreshTime;
                                cron += ' * * * * *';
