...
export class NetworksDeleteDialogComponent implements OnInit {
    ...
    save() {
        ...
        forkJoin(obs).subscribe(resps => {
            ...
            if (ok) {
                this.snackBar.open('Hub ' + (ids.length > 0 ? 'and devices ' : '') + 'deleted successfully.', undefined, {duration: 2000});
                this.close(true);
            } else {
                this.snackBar.open('Error while deleting the hub' + (ids.length > 0 ? ' and devices' : '') + '!',
