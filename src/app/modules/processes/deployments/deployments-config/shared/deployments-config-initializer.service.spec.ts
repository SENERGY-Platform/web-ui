...
describe('DeploymentsConfigInitializerService', () => {
    ...
    it('init FormControl with Lane Deployment, disable device and service selection for 2 out of 3 tasks', inject(
        ...
        (service: DeploymentsConfigInitializerService) => {
            ...
            expect((formGroup.get(['elements', 0, 'task', 'selection', 'selected_device_id']) as FormControl).disabled).toBe(false);
            expect((formGroup.get(['elements', 0, 'task', 'selection', 'selected_service_id']) as FormControl).disabled).toBe(false);
            expect((formGroup.get(['elements', 2, 'task', 'selection', 'selected_device_id']) as FormControl).disabled).toBe(true);
            expect((formGroup.get(['elements', 2, 'task', 'selection', 'selected_service_id']) as FormControl).disabled).toBe(true);
            expect((formGroup.get(['elements', 4, 'task', 'selection', 'selected_device_id']) as FormControl).disabled).toBe(true);
            expect((formGroup.get(['elements', 4, 'task', 'selection', 'selected_service_id']) as FormControl).disabled).toBe(true);
        },
    ));

    it('init FormControl with MessageEvent', inject(
        [DeploymentsConfigInitializerService],
        (service: DeploymentsConfigInitializerService) => {
